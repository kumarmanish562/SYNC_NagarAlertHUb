                        ...
                        <form className="space-y-6" onSubmit={handleReset}>
                            <div className="space-y-2">
                                <label className="text-xs font-semibold text-slate-500 dark:text-gray-400 uppercase tracking-wider">
                                    {userType === 'citizen' ? 'Mobile Number' : 'Registered Email ID'}
                                </label>
                                <div className="relative">
                                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-slate-400 dark:text-gray-500">
                                        {userType === 'citizen' ? (
                                            <span className="text-slate-700 dark:text-white text-sm font-medium pr-1">+91</span>
                                        ) : (
                                            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
                                        )}
                                    </div>
                                    <input
                                        type={userType === 'citizen' ? 'tel' : 'email'}
                                        placeholder={userType === 'citizen' ? '98765 43210' : 'official@nagaralert.gov.in'}
                                        className={`block w-full ${userType === 'citizen' ? 'pl-12' : 'pl-10'} pr-3 py-3 border border-slate-200 dark:border-slate-700 rounded-lg leading-5 bg-slate-50 dark:bg-slate-800/50 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all sm:text-sm`}
                                    />
                                </div>
                            </div>
